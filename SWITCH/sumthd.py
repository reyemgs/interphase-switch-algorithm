    ...
    for i in range(1, numof_value, 2):
        ...
        if vec_x[a] == 1:
            if vec_x[i] == 0:
                ...
                #wf_sum[2,] = wf_sum[2,] + wf_vector[int(v_struct[g,1]),:]
                #wf_sum[2,] = wf_sum[2,] + wf_vector[0,]

        else:
            if vec_x[i] == 1:
                dec_vector[g] = 2
                wf_sum[1,] = wf_sum[1,] + wf_vector[int(v_struct[1,g]),]
                #wf_sum[1,] = wf_sum[1,] + wf_vector[int(v_struct[g,1]),:]
                #wf_sum[1,] = wf_sum[1,] + wf_vector[1,]

            else:
                dec_vector[g] = 1
                wf_sum[0,] = wf_sum[0,] + wf_vector[int(v_struct[1,g]),]
                #wf_sum[0,] = wf_sum[0,] + wf_vector[int(v_struct[g,1]),:]
                #wf_sum[0,] = wf_sum[0,] + wf_vector[2,]
